...
local function SafeGetThemeTable()
    local tbl = nil
    pcall(function()
        if typeof(readfile) == 'function' and typeof(isfile) == 'function' and isfile('VisualUILibraryCurrentTheme.json') then
            tbl = HttpService:JSONDecode(readfile('VisualUILibraryCurrentTheme.json'))
        end
    end)
    if not tbl and CurrentThemeTable then tbl = CurrentThemeTable end
    if not tbl then
        local def = {}
        for k, v in next, Themes.Default do
            def[k] = Utility:SplitColor(v)
        end
        tbl = def
    end
    return tbl
...
do
    ...
    function Utility:DestroyUI()
        ChangeTheme = true
        BreakAllLoops = true
        pcall(function() if typeof(writefile) == 'function' then writefile('fml.txt', 'true') end end)
...
function Library:CreateWindow(HubName, GameName, IntroText, IntroIcon, ImprovePerformance, ConfigFolder, Theme)
    ...
    pcall(function()
        if typeof(isfile) == 'function' and typeof(delfile) == 'function' and isfile('VisualUILibraryCurrentTheme.json') then
